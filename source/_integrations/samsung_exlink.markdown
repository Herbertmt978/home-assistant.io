...
- The TV must be powered on for the initial setup, because Home Assistant queries the TV to confirm the connection. A TV in standby does not answer status queries.
...
1. Make sure the TV is powered on. The TV must be on for the initial connection, because a TV in standby does not answer status queries.
2. On TVs that use the USB-to-ExLink dongle, make sure **EXT Link Support** and **USB Serial** are enabled in the TV's service menu (open it with **Mute → 1 → 8 → 2 → Power**).
3. Verify the cable is connected to the TV's 3.5&nbsp;mm ExLink jack (or the Samsung USB-to-ExLink dongle) and is fully seated.
